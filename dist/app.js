...
document.addEventListener("dragover", function(event) {
  ...
  if (event.target.classList.contains("drop-area")) {
    dragged.parentNode.classList.add("hover");
    event.target.querySelector('span').classList.add("hover");
  }
}, false);
...
document.addEventListener("dragleave", function(event) {
  ...
  if (event.target.classList.contains("drop-area")){
    event.target.querySelector('span').classList.remove("hover");
  }
}, false);
...
document.addEventListener("drop", function(event) {
    ...
    var drop_name = dragged.dataset.name;
    var drop_value = event.target.innerText;
    var img_select = document.getElementsByName(drop_name)[0];

    dragged.parentNode.style.background = "none";
    event.target.style.background = "none";
    //remove image from another node if already added
    var nodes = document.querySelectorAll(".map #"+drop_name);
    for (var i = 0; i < nodes.length; i++) {
      nodes[i].remove();
    }

    img_select.value = drop_value;

  // prevent default action (open as link for some elements)
  event.preventDefault();
  // copy dragged elem to the selected drop target
  if (event.target.classList.contains("drop-area") ) {
    var dragCopy = dragged.cloneNode(true);
    dragCopy.style.opacity = 1;
    //clear node if there, reset select value
    // remove all child images
    var images = event.target.getElementsByTagName('img');
    while(images.length > 0) {
        images[0].parentNode.removeChild(images[0]);
    }

    event.target.appendChild( dragCopy );
  }
}, false);
...
window.onload = function() {
    ...
    var toggles = document.getElementsByClassName("toggle-hidden");
...
//     var vid = document.querySelector(location.hash + " video");
//     if (vid) {
